...
def generate_white_noise(sample):
    return np.random.normal(size=sample)
...
if __name__=='__main__':
    ...
    f=open(f'record_shs_3-3_p={low}{high}.txt','a',encoding='UTF-8')
    f.write('shs_ratio '+'snr'+'\n')
    g=open(f'record_fovar_3-3_p={low}{high}.txt','a',encoding='UTF-8')
    g.write('fovar_ratio '+'snr'+'\n')
    h=open(f'record_total_3-3_p={low}{high}.txt','a',encoding='UTF-8')
    ...
    for a in range(6):
        ...
        for i in range(total_box.size):
            #ピンクノイズの生成
            noise_amp = generate_white_noise(noise_length)
            ...
            t_n, shs_array_n, f0_array_n = calc_f0_shs(sample_rate,adjusted_noise_amp)

            #基本周波数分散の計算
            f0_var_array=cal_f0_continuity(f0_array)
            f0_var_array_n=cal_f0_continuity(f0_array_n)
            ...
            for j in range(np.size(shs_array)):
                if shs_array[j]>n_shs_percentile:
                    score1+=1
            ...
            #基本周波数連続時間率の計算
            n_var_percentile=np.percentile(f0_var_array_n,low)
            score2=0
            for j in range(np.size(f0_var_array)):
                if f0_var_array[j]<n_var_percentile:
                    score2+=1
    ...
    #グラフ作成用ファイル
    e=open(f'forgraph_shs_3-3_p={high}{low}.txt','a',encoding='UTF-8')
    ...
    e=open(f'forgraph_fovar_3-3_p={high}{low}.txt','a',encoding='UTF-8')
    for a in range(len(snr_box)):
        e.write(str(snr_box[a])+' ')
    e.write('\n')
    ...
    e=open(f'forgraph_total_3-3_p={high}{low}.txt','a',encoding='UTF-8')
    for a in range(len(snr_box)):
        e.write(str(snr_box[a])+' ')
    e.write('\n')
